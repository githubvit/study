...
class UIFunctions(MainWindow):
    ...
    ## ==> TOGGLE MENU  点击TOGGLE按钮 边栏菜单展开/收拢 (展开最大宽度,是否展开)
    ########################################################################
    ...
    def addNewMenu(self, name, objName, icon, isTopMenu):
        ...
        if isTopMenu:
            ...
            # layout_menus 是布局的名称不是控件的名称,查阅左侧的空白frame控件 frame_menus ,在其属性的 layout 项目下的layoutName即是layout_menus.
            # 因为我们是向布局增加控件，不是向控件增加控件因此不能用 控件名称，必须用布局名称。
        else:
    ...
    def uiDefinitions(self):
        def dobleClickMaximizeRestore(event):
            # IF DOUBLE CLICK CHANGE STATUS 双击标题栏
            if event.type() == QtCore.QEvent.MouseButtonDblClick:
                QtCore.QTimer.singleShot(250, lambda: UIFunctions.maximize_restore(self))

        ## REMOVE ==> STANDARD TITLE BAR 标题栏
        if GLOBAL_TITLE_BAR:
            self.setWindowFlags(QtCore.Qt.FramelessWindowHint) # 无边框
            self.setAttribute(QtCore.Qt.WA_TranslucentBackground) # 背景透明
            self.ui.frame_label_top_btns.mouseDoubleClickEvent = dobleClickMaximizeRestore # 双击标题栏还原
        else:
            self.ui.horizontalLayout.setContentsMargins(0, 0, 0, 0)
        ...
        ## ==> RESIZE WINDOW 窗口伸缩
        self.sizegrip = QSizeGrip(self.ui.frame_size_grip) # QSizeGrip 提示窗口拉伸标志
