...
class Window(QWidget):
    ...
    def 同步滚动行号案例(self):
        ...
        self.r_frame.resize(30,self.pte.height())
        margin=2
        self.r_frame.move(self.pte.x()-self.r_frame.width()-margin,self.pte.y())
        # self.r_frame.setStyleSheet('border:1px solid red')
        ...
        self.rlb.setStyleSheet('border:1px solid red')
        # 填写行号
        self.add_rindex()
        ...
        def scroll_label(rect, dy):
            ...
            self.rlb.move(self.rlb.x(), self.rlb.y() + dy) # 滚动标签
    ...
    def add_rindex(self):
        # 获取文本行数
        # （小技巧：用.document().size().height()获取的文本高度就是 行数，
        ...
        while rheight<self.pte.height():
            rstr += str(rnum)+'\n'
            rnum+=1
            rheight+=10 # 这样设置是知道一定会超过文本框的高度一点。因为1行要大于10像素高。 而只要文本标签的行数大于文本框即可。

        # 如果文本的行数，大于文本标签的行数，标签的行号就应用文本的行数
        if dr_num>rnum:
    ...
    def 信号的操作(self):
        ...
        # self.pte.blockCountChanged.connect(lambda bc:print("块的个数改变", bc))#bc即换行数，回车数
        # self.pte.selectionChanged.connect(lambda :print("选中内容发生了改变", self.pte.textCursor().selectedText()))
        
        # self.pte.modificationChanged.connect(lambda val:(print("修改状态发生改变", val),print('多行lambda 2'),self.pte.document().setModified(False))) # 这样不会有反映
        # self.pte.modificationChanged.connect(lambda val:(print("修改状态发生改变", val),print('多行lambda 2')))
        # 
    ...
    def 放大缩小(self):
        ...
        btn1.adjustSize()
        btn1.move(50,10)
        btn1.setAutoRepeat(True) #按住不放可以连续放大 无级变速
    ...
    def 块的操作(self):
        ...
        self.pte.setMaximumBlockCount(2) # 设置最大块个数 最大段落数 留下最近的 删除最远的
        print(self.pte.toPlainText()) # 获取文档的文本 是 删除了多余段落的文本
