...
# 打包成单个.exe 时用，因为没有目录了，就用'main.exe'上一级目录。 然而打包后还是没有目录
# rpath=os.path.dirname(os.path.dirname(__file__)) # 失败

# 解决 文件分隔符 读取为'\\'和'/'的问题
# if len(os.path.dirname(__file__).split('/')) ==1:
#     rpath=os.path.dirname(__file__).split('\\')[0]
# else:
#     rpath=os.path.dirname(__file__).split('/')[0]

# 更简单 反正就取一个盘符就用':'分割
# 如果 d盘存在就用 否则 取程序所在的盘
if os.path.exists('d:/'):
    rpath='d:/mmx'
else:
    rpath=os.path.dirname(__file__).split(':')[0]+':/mmx'
...
def smt_encrypt_dir():
    encrypt_dir=rpath+'/encrypt_file'
    if not os.path.exists(encrypt_dir):
        os.makedirs(encrypt_dir)
    return encrypt_dir

def smt_decrypt_dir():
    decrypt_dir=rpath+'/decrypt_file'
...
def build_keyfile_by_name(name):
    ...
    # 2 判断密钥文件是否已经存在如果存在，就退出
    # 如果公钥已经存在，那么与其配套的私钥一定存在，不管这个私钥在哪里，都不能再用这个名字命名密钥文件了。
...
def encrypt_file(filepath,pub_key_file):
    ...
    # 3.2 取得公钥名
    keyname=pub_key_file.split('/')[-1].split('_')[0]
    # 3.3 设置加密文件名 
    encrypt_filename='encrypt_'+keyname+'_'+filename
...
def file_encrypt(filepath):
    ...
    pubfile=rpath+'/key/ub/'+filename2+'_public.pem'
...
def decrypt_file(encryptfile,prikey_file):
    ...
    # 3.1 取得文件名
    filename=encryptfile.split('/')[-1].split('_')[-1]
    # 3.2 判断指定目录是否存在
    decrypt_file_path=rpath+'/decrypt_file'
    ...
    # 3.3 将 解密结果 写入
    with open(decrypt_file_path+'/'+filename,'wb') as f:
        f.write(res_decrypt) 
