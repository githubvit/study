...
def normalizeRect(rect):
    ...
    w = rect.width()
    h = rect.height()
    # 将任何起始点框选 统一转换为 左上 为起始点   而框本身位置大小不变
    if w < 0:
        x = x + w
        w = -w
    if h < 0:
...
class WScreenshot(QWidget):
    ...
    def __init__(self, parent = None):
        ...
        self.setWindowFlags(Qt.WindowStaysOnTopHint | Qt.FramelessWindowHint)   # 窗体风格为：总在最上面的窗口,置顶 Qt.WindowStaysOnTopHint |无边框 Qt.FramelessWindowHint
        
        # 屏幕 和 屏幕信息
        self.screen = QApplication.primaryScreen().grabWindow(QApplication.desktop().winId()) # 获取 主屏幕桌面截图
        ...
        palette = QPalette()
        palette.setBrush(QPalette.Background, QBrush(self.screen)) # 用调色板 设置 背景 为 截图self.screen 
        self.setPalette(palette)

        # 蒙板层
        # 调节器层
        self.adjustment_original = QPixmap(self.screenSize)  # 初始调节器 蒙板大小 截图尺寸self.screenSize
        self.adjustment_original.fill(QColor(0, 0, 0, 64))  # 设置蒙板
        ...
        self.canvas_original.fill(Qt.transparent) # 全透明
        ...
        self.setMouseTracking(True) #设置鼠标跟踪 感应 光标形状
    ...
    def toMask(self):
        ...
        if self.isShifting: #按下Shift 同比例 缩放 
            x = rect.x()
            y = rect.y()
            w = rect.width()
            h = rect.height()
